...
export default function LibrariesScreen() {
  ...
  const activeLibrary = selectedLibrary ?? libraries[0] ?? null;
  ...
  const {
    ...
    queryKey: ['library-books', activeLibrary?.id],
    queryFn: () => getLibraryBooks(activeLibrary!.id),
    enabled: !!activeLibrary,
  });

  const onRefresh = async () => {
    setRefreshing(true);
    await Promise.all([refetchLibraries(), refetchBooks()]);
    setRefreshing(false);
  };

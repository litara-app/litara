...
${doc.querySelector('parsererror').innerText}`)
    ...
    resolveHref(href) {
        ...
        if (!item) { console.warn(`[foliate] resolveHref: no manifest item for path "${path}"`); return null }
        const index = this.resources.spine.findIndex(({ idref }) => idref === item.id)
        if (index === -1) console.warn(`[foliate] resolveHref: item "${item.id}" (${path}) found in manifest but not in spine`)

...
:::warning Beta
The Litara mobile app is in **beta**. Expect rough edges, missing features, and breaking changes between releases. It is not yet suitable for everyday use.
:::
...
| Libraries | Browse by library folder; when multiple libraries exist, a tab strip at the top lets you switch between them — each tab shows the library's icon (if set) and name |
